...
=item Temma->process_html ($input, $args => $fh, $cb)
...
The second argument must be the hash reference containing arguments to
the template represented as zero or more name-value pairs.

The third argument must be the object which has the C<print> method
that receives the character string to be written.

The four argument must be the code reference, which is invoked when
...
=item Temma->process_html_fragment ($input, $args => $fh, $cb)
...
=item Temma->process_plain_text ($input, $args => $fh, $cb)
...
<https://github.com/wakaba/perl-charclass>
